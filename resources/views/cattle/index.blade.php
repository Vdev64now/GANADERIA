            ...
            <table class="custom-table">
                ...
                <tbody>
                    @forelse($cattles as $item)
                        <tr>
                            <td data-label="Arete (ID)" style="font-weight: 600; color: #ffffff;">{{ $item->ear_tag }}</td>
                            <td data-label="Hacienda">{{ $item->farm->name }}</td>
                            <td data-label="Raza">{{ $item->breed ?? 'N/A' }}</td>
                            <td data-label="Proveedor">{{ $item->provider ?? 'N/A' }}</td>
                            <td data-label="Fecha Compra">{{ $item->purchase_date ? $item->purchase_date->format('d/m/Y') : 'N/A' }}</td>
                            <td data-label="Peso en Pie" style="font-weight: 500;">{{ number_format($item->live_weight, 2) }} kg</td>
                            <td data-label="Precio Compra" style="font-weight: 500;">${{ number_format($item->purchase_price_total, 2) }}</td>
